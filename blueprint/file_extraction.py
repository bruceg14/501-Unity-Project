...
cursor.execute("SELECT graph_uuid FROM graph WHERE graph_type = 'UbergraphPage' ORDER BY RANDOM() LIMIT 10000")
...
for graph in randomly_selected_graph_uuids:
    ...
    edges = []
    has_branching_node = False
    for pin1_uuid, pin2_uuid, pin2_owningnode in rows:
        cursor.execute("SELECT OwningNode, pin_direction from pin WHERE pin_uuid = ?", (pin1_uuid, ))
        node1_uuid, pin_direction = cursor.fetchone()

        edges.append((node1_uuid, pin_direction, pin2_owningnode))
    ...
    for node_uuid, node_type in rows:
        ...
        # if node_type in ["IfThenElse"]:
        #     has_branching_node = True
        has_branching_node = True
        # print(node_uuid, node_type, node_pos_x, node_pos_y)
        
        nodes.append((node_uuid, node_type, node_pos_x, node_pos_y))
    if has_branching_node:
